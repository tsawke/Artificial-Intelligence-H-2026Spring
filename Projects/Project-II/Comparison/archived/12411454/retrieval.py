...
class Retrieval:
    ...
    def _hybrid_topk(
        ...
        raw_keep = 4
        raw_candidates = np.argpartition(raw_distances, kth=self.k - 1, axis=1)[:, : self.k]
        row = np.arange(raw_candidates.shape[0])[:, None]
        raw_order = np.argsort(raw_distances[row, raw_candidates], axis=1)
        raw_top = raw_candidates[row, raw_order]

        primary_top = self._ordered_top_candidates(primary_distances, 10)
        far_top = self._ordered_top_candidates(far_distances, 10)
        transform_top = self._ordered_top_candidates(transform_distances, 10)
        blur_top = self._ordered_top_candidates(blur_distances, 10)
        ...
        for i in range(raw_distances.shape[0]):
            ...
            alternatives = []
            for idx in primary_top[i]:
                item = int(idx)
                if item not in used:
                    ratio = float(primary_distances[i, item]) / raw_next_distance
                    if ratio <= self.shift_replacement_margin:
                        alternatives.append((ratio, item))
                    break
            for idx in far_top[i]:
                item = int(idx)
                if item not in used:
                    ratio = float(far_distances[i, item]) / raw_next_distance
                    if ratio <= self.far_shift_replacement_margin:
                        alternatives.append((ratio, item))
                    break
            for idx in transform_top[i]:
                item = int(idx)
                if item not in used:
                    ratio = float(transform_distances[i, item]) / raw_next_distance
                    if ratio <= self.transform_replacement_margin:
                        alternatives.append((ratio, item))
                    break
            for idx in blur_top[i]:
                item = int(idx)
                if item not in used:
                    ratio = float(blur_distances[i, item]) / raw_next_distance
                    if ratio <= self.blur_replacement_margin:
                        alternatives.append((ratio, item))
                    break
            if alternatives:
                _, best_item = min(alternatives)
                selected.append(best_item)
                used.add(best_item)
            else:
                selected.append(raw_next)
                used.add(raw_next)
